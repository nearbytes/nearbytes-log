 ...
 */
export async function loadEventLog(channel, log, crypto) {
 ...
 */
export async function verifyEventLog(entries, channel, crypto) {
    for (const entry of entries) {
        const payloadBytes = serializeEventEnvelope(entry.signedEvent.envelope);
        const isValid = await crypto.verifyPU(payloadBytes, entry.signedEvent.signature, channel.publicKey);
        if (!isValid) {
